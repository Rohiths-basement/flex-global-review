...
export async function GET(req: NextRequest) {
  try {
    ...
    if (parsed.categoryName) {
      ...
      items = items.filter((r) => {
        const reviewWithCategories = r as { categories?: Record<string, number> };
        const v = reviewWithCategories.categories ? reviewWithCategories.categories[cat] : undefined;
  ...
  const window = items.slice(parsed.offset, parsed.offset + parsed.limit);
    ...
    console.error('Error in hostaway reviews:', error);
    return NextResponse.json({ 
      status: "error", 
